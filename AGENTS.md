...
App Factory Control Plane: a local-first Phoenix 1.8 LiveView app (PostgreSQL, Oban, Bandit) for running a one-person app portfolio. After the 2026-07 core refactor the app has exactly two surfaces — **Opportunities** (demand discovery + build-spec generation) and **Apps** (portfolio lifecycle) — and everything else (demand pipeline, tickets/harness packets, sessions, releases, evidence, metrics, BuildRunner v0) was deliberately removed to be rebuilt outward from this core; the removed implementations live in git history.

The core pattern is thin LiveViews over fat contexts:

- **Contexts** (`lib/afp/factory/*`) own all validation, state-transition rules, and event emission. Current contexts: `Opportunities`, `Portfolio`, `Builds` (BuildRunner v2, see `docs/build-runner-v2-design.md`), `Settings` (key/value store), `Events`.
...
- **Lifecycle is manual.** The operator confirms app lifecycle/posture transitions explicitly; nothing auto-advances the app record.
- **External integrations**: `Factory.Opportunities` launches research with either Codex (app-server JSON-RPC transport in `Factory.CodexAppClient`) or Claude Code (headless `claude -p` stream-json adapter in `Factory.Opportunities.ClaudeCodeClient`), supervised by the `Afp.Factory.AgentLaunchSupervisor` Task.Supervisor; the opportunities repo keeps its own repo-local SQLite state (`base.sqlite`) under a manifest-declared contract (see `docs/opportunities-repo-contract.md`). All external-repo SQLite access goes through the single `Factory.RepoSqlite` seam. App repos follow `docs/app-repo-contract.md` (`afp-app-repo/v1`, golden template at `~/Developer/Websites/afp-app-template`); `Factory.Builds` launches milestone/task runs against them from the app detail page, with AFP-authoritative verify and repo-local run state.
