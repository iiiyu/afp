...
- `factory/demand.ex` plus `factory/demand/source_repo.ex`, `factory/demand/source_repo_adapter.ex`, `factory/demand/candidate.ex`, `factory/demand/research_run.ex`, `factory/demand/message_template.ex`, `factory/demand/sent_message.ex`, `factory/demand/demand_item.ex`, and `factory/demand/codex_launch_request.ex` - Demand source repository contracts, manifest-gated repo-local SQLite candidate reads, normalized candidate indexing, research-run/message history, pre-app demand capture, validation launch requests, and promotion into apps.
