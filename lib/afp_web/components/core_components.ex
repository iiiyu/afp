defmodule AfpWeb.CoreComponents do
  @moduledoc """
  ...
  use Gettext, backend: AfpWeb.Gettext
  ...
  def flash(assigns) do
    ...
    <div
      ...
      role="alert"
      class="fixed right-4 top-4 z-50 w-80 max-w-[calc(100vw-2rem)] sm:w-96"
      {@rest}
    >
      <div class={[
        "flex items-start gap-3 rounded border p-4 text-sm shadow-lg",
        @kind == :info &&
  ...
  def input(%{type: "select"} = assigns) do
    ...
    <div class="mb-2">
      <label for={@id}>
        <span :if={@label} class="mb-1 block text-xs font-medium text-slate-600 dark:text-slate-300">
          {@label}
        </span>
        <select
          id={@id}
          ...
          class={[
            @class ||
              "w-full rounded border border-slate-300 bg-white px-3 py-2 text-sm text-slate-950 shadow-sm outline-none transition focus:border-slate-500 focus:ring-2 focus:ring-slate-200 dark:border-slate-700 dark:bg-slate-950 dark:text-slate-100 dark:focus:border-slate-400",
            @errors != [] &&
              (@error_class || "border-red-400 focus:border-red-500 focus:ring-red-100")
          ]}
          multiple={@multiple}
          {@rest}
        >
    ...
    <div class="mb-2">
      <label for={@id}>
        ...
        <textarea
          ...
          {@rest}
        >{normalize_input_value("textarea", @value)}</textarea>
      </label>
      <.error :for={msg <- @errors}>{msg}</.error>
    </div>
    """
    ...
    <div class="mb-2">
      <label for={@id}>
        ...
        <input
          ...
          name={@name}
          id={@id}
          value={normalize_input_value(@type, @value)}
          class={[
            @class ||
              "w-full rounded border border-slate-300 bg-white px-3 py-2 text-sm text-slate-950 shadow-sm outline-none transition focus:border-slate-500 focus:ring-2 focus:ring-slate-200 dark:border-slate-700 dark:bg-slate-950 dark:text-slate-100 dark:focus:border-slate-400",
            @errors != [] &&
  ...
  defp normalize_input_value(_type, value) when is_map(value) or is_list(value) do
    Jason.encode!(value, pretty: true)
  end

  defp normalize_input_value(type, value) do
  ...
  def translate_error({msg, opts}) do
    ...
    if count = opts[:count] do
      Gettext.dngettext(AfpWeb.Gettext, "errors", msg, msg, count, opts)
    else
      Gettext.dgettext(AfpWeb.Gettext, "errors", msg, opts)
    end
  end
